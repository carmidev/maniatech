...
https.get(url, { headers: { 'apikey': apikey, 'Authorization': `Bearer ${apikey}` } }, (res) => {
  ...
  res.on('end', () => {
    try {
      ...
      console.log('Tables:', Object.keys(result.definitions));
      if (result.definitions.products) {
        console.log('Products:', result.definitions.products.properties);
      }
      if (result.definitions.inventory) {
        console.log('Inventory:', result.definitions.inventory.properties);
      }
    } catch (err) {
